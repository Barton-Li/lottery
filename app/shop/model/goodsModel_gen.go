...
type (
	...
	Goods struct {
		...
		Precoupon_Price      float64 `db:"precoupon _price"`
		Aftercoupon_Price    float64 `db:"aftercoupon _price"`
		GoodsDesc            string  `db:"goods_desc"`
		WishPoints           int64   `db:"wish_points"`
		CouponStartTime      int64   `db:"coupon_start_time"`
		CouponEndTime        int64   `db:"coupon_end_time"`
		CouponDiscount       int64   `db:"coupon_discount"`
		CouponRemainQuantity int64   `db:"coupon_remain_quantity"`
	}
)
...
func (m *defaultGoodsModel) Insert(ctx context.Context, data *Goods) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.GoodsId, data.CategoryId, data.Precoupon_Price, data.Aftercoupon_Price, data.GoodsDesc, data.WishPoints, data.CouponStartTime, data.CouponEndTime, data.CouponDiscount, data.CouponRemainQuantity)
	}, shopGoodsIdKey)
	return ret, err
}
...
func (m *defaultGoodsModel) TransInsert(ctx context.Context, session sqlx.Session, data *Goods) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("insert into %s (%s) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", m.table, goodsRowsExpectAutoSet)
		return session.ExecCtx(ctx, query, data.GoodsId, data.CategoryId, data.Precoupon_Price, data.Aftercoupon_Price, data.GoodsDesc, data.WishPoints, data.CouponStartTime, data.CouponEndTime, data.CouponDiscount, data.CouponRemainQuantity)
	}, shopGoodsIdKey)
	return ret, err
}
func (m *defaultGoodsModel) Update(ctx context.Context, data *Goods) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.GoodsId, data.CategoryId, data.Precoupon_Price, data.Aftercoupon_Price, data.GoodsDesc, data.WishPoints, data.CouponStartTime, data.CouponEndTime, data.CouponDiscount, data.CouponRemainQuantity, data.Id)
	}, shopGoodsIdKey)
	return err
}
...
func (m *defaultGoodsModel) TransUpdate(ctx context.Context, session sqlx.Session, data *Goods) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("update %s set %s where `id` = ?", m.table, goodsRowsWithPlaceHolder)
		return session.ExecCtx(ctx, query, data.GoodsId, data.CategoryId, data.Precoupon_Price, data.Aftercoupon_Price, data.GoodsDesc, data.WishPoints, data.CouponStartTime, data.CouponEndTime, data.CouponDiscount, data.CouponRemainQuantity, data.Id)
	}, shopGoodsIdKey)
	return err
}
