...
var (
	...
	cacheUsercenterUserShopIdPrefix = "cache:usercenter:userShop:id:"
)
...
type (
	...
	UserShop struct {
		Id         int64        `db:"id"`          // 主键，店铺记录唯一标识符，自动递增
		UserId     int64        `db:"user_id"`     // 用户ID，关联到user_info表中的id
		Name       string       `db:"name"`        // 店铺名称，默认为空字符串
		Location   float64      `db:"location"`    // 店铺位置，以经纬度格式存储
		CreateTime time.Time    `db:"create_time"` // 店铺记录创建时间，默认为当前时间
		UpdateTime time.Time    `db:"update_time"` // 店铺记录最后更新时间，默认为当前时间，并且在每次更新时自动更新为当前时间
		DeleteTime sql.NullTime `db:"delete_time"` // 店铺记录删除时间，默认为空
	}
)
...
func (m *defaultUserShopModel) Delete(ctx context.Context, id int64) error {
	usercenterUserShopIdKey := fmt.Sprintf("%s%v", cacheUsercenterUserShopIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, usercenterUserShopIdKey)
	return err
}
...
func (m *defaultUserShopModel) FindOne(ctx context.Context, id int64) (*UserShop, error) {
	usercenterUserShopIdKey := fmt.Sprintf("%s%v", cacheUsercenterUserShopIdPrefix, id)
	var resp UserShop
	err := m.QueryRowCtx(ctx, &resp, usercenterUserShopIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultUserShopModel) Insert(ctx context.Context, data *UserShop) (sql.Result, error) {
	usercenterUserShopIdKey := fmt.Sprintf("%s%v", cacheUsercenterUserShopIdPrefix, data.Id)
...
func (m *defaultUserShopModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheUsercenterUserShopIdPrefix, primary)
}
